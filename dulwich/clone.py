...
def do_clone(
    ...
    try:
        ...
        head_ref = _set_default_branch(target, origin, origin_head, branch, ref_message)
...
def _set_default_branch(r, origin, origin_head, branch, ref_message):
    ...
    if branch:
        ...
        if origin_ref in r.refs:
            ...
            r.refs.add_if_new(local_ref, r.refs[origin_ref], ref_message)
            head_ref = local_ref
        elif LOCAL_TAG_PREFIX + branch in r.refs:
            head_ref = LOCAL_TAG_PREFIX + branch
        else:
            raise ValueError("%s is not a valid branch or tag" % os.fsencode(branch))
    elif origin_head:
        ...
        try:
            r.refs.add_if_new(head_ref, r.refs[origin_ref], ref_message)
        except KeyError:
            pass
    return head_ref
...
def _set_head(r, head_ref, ref_message):
    if head_ref.startswith(LOCAL_TAG_PREFIX):
        ...
        del r.refs[b"HEAD"]
        r.refs.set_if_equals(b"HEAD", None, head, message=ref_message)
    else:
